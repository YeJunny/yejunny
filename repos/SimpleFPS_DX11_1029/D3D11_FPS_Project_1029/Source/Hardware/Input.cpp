...
bool Input::Initialize(int screenWidth, int screenHeight)
{
	...
	HRESULT result = DirectInput8Create(Global::GetHinst(), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&m_directInput, NULL);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_keyboard->SetCooperativeLevel(Global::GetHwnd(), DISCL_FOREGROUND | DISCL_EXCLUSIVE);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_mouse->SetCooperativeLevel(Global::GetHwnd(), DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
	if (FAILED(result))
	{
		return false;
	}
